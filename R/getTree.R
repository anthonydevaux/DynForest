...
#' @param tree_id Integer indicating the tree identifier
#'
...
#' res_tree4 <- getTree(DynForest_obj = res_dyn, tree_id = 4)
#' }
#' @export
getTree <- function(DynForest_obj, tree_id){

  if (!methods::is(DynForest_obj,"DynForest")){
    stop("'DynForest_obj' should be a 'DynForest' class!")
  }

  if (!inherits(tree_id, "numeric")){
    stop("'tree_id' should be a numeric object containing the tree identifier!")
  }

  if (!any(tree_id==seq(DynForest_obj$param$ntree))){
    stop(paste0("'tree_id' should be chosen between 1 and ", DynForest_obj$param$ntree, "!"))
  }

  out <- DynForest_obj$rf[,tree_id]$V_split
